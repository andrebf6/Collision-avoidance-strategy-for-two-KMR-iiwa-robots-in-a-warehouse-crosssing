...
def main():
    ...
    parser.add_argument('-sdf', '--robot_sdf', type=str, default='dummy.urdf',
    ...
    parser.add_argument('-ox', type=float, default=0,
                        help='x component quaternion for the orientation [radians]')
    parser.add_argument('-oy', type=float, default=0,
                        help='y component quaternion for the orientation [radians]')
    parser.add_argument('-oz', type=float, default=0,
                        help='z component quaternion for the orientation [radians]')
    parser.add_argument('-ow', type=float, default=0,
                        help='w component quaternion for the orientation [radians]')
    ...
    sdf_file_path = os.path.join(get_package_share_directory('kmr_model'), 'robot/', 'model.sdf')
    ...
    request.initial_pose.orientation.x = float(args.ox)
    request.initial_pose.orientation.y = float(args.oy)
    request.initial_pose.orientation.z = float(args.oz)
    request.initial_pose.orientation.w = float(args.ow)


    if args.namespace is True:
        node.get_logger().info('spawning `{}` on namespace `{}` at {}, {}, {}'.format(
            args.robot_name, args.robot_namespace, args.x, args.y, args.z, args.ox, args.oy, args.oz, args.ow))
        ...
        node.get_logger().info('spawning `{}` at {}, {}, {}'.format(
            args.robot_name, args.x, args.y, args.z, args.ox, args.oy, args.oz, args.ow))
