...
def listener_callback2(odom_data2):
    ...
    pr2[0]=xr2; pr2[1]=yr2; yaw2[0]= yawc2
    v2[0]=vxr2; v2[1]=vyr2
...
if __name__=="__main__":# Proteger parte del código. Assegura que el bloque solo se ejecutará si tu módulo es el programa principal.
    ...
    pe=np.ones(2);pe[0]=-0.5; pe[1]=1.1
    ...
    v2=np.ones(2)*2000                              # Velocity of the robot 2's centre of mass in the odom frame  (m)
    yaw2=np.ones(1)*2000                            # Orientation of the robot 1's in the 2D plane (rad)
    rotz2=np.zeros((2,2))*2000                      # Initialize rotation matrix (change from the robot 2 base-footprint frame to the odom frame).
    ...
    try: # The try block lets you test a block of code for errors.
        ...
        waypoints[0,0]=0.8; waypoints[0,1]=0.9
        waypoints[1,0]=2.5; waypoints[1,1]=2.1
        waypoints[2,0]=5; waypoints[2,1]=2.1
            ...
            if kk ==0:          # Collision detection activated
                while(1):
                    ...
                    if pr1[0]!=2000 and pr1[1]!=2000 and scan==True : # Position information has been published at least once and a new sensor reading has entred.
                        ...
                        uu=phi//6.28318530718; phi=phi-uu*6.28318530718
                        ...
                        le1=pe-pl1                                                      # Build cone of vision in the odom frame
                        la1=pa-pl1
                        calpha1=(np.dot(le1,dv1_o))/(LA.norm(le1)*(LA.norm(dv1_o))); alpha1=math.acos(calpha1) 
                        calpha2=(np.dot(la1,dv1_o))/(LA.norm(la1)*(LA.norm(dv1_o))); alpha2=math.acos(calpha2)
                        ...
                        delt1=alpha1
                        cdelt2=(np.dot(pb-pl1,dv1_o))/(LA.norm(pb-pl1)*(LA.norm(dv1_o))); delt2=math.acos(cdelt2)
                        cdelt3=(np.dot(pb-pl1,pa-pl1))/(LA.norm(pb-pl1)*(LA.norm(pa-pl1))); delt3=math.acos(cdelt3)
                        
                        id3=np.where((phi-delt3-delt2<= oi_angles) & (oi_angles<phi-delt2))
                        if id3[0][0]!=0:
                            ff=np.arange(0,id3[0][0])
                            id3=np.concatenate((ff,id3[0]))
                        id2=np.where((phi-delt2<= oi_angles) & (oi_angles < phi))
                        id1=np.where((phi<= oi_angles) & (oi_angles <= phi+delt1))
                        if id1[0][-1]!=(len(oi_angles)-1):
                            vv=np.arange(id1[0][-1]+1,len(oi_angles))
                            id1=np.concatenate((id1[0],vv))

                        oi_angles_d3=oi_angles[id3]
                        oi_angles_d2=oi_angles[id2]
                        oi_angles_d1=oi_angles[id1]
                        
                        cdd_d3=(y_lim-pl1[1])/np.sin(oi_angles_d3)
                        cdd_d2=(x_lim2-pl1[0])/np.cos(oi_angles_d2); cdd_d2=abs(cdd_d2)
                        cdd_d1=(x_lim1-pl1[0])/np.cos(oi_angles_d1); cdd_d1=abs(cdd_d1)
                        cdd=np.concatenate((cdd_d3,cdd_d2,cdd_d1))
                        
                        #Detect possible collsision
                        if np.any(i_readings-cdd < 0)==True:

                            #Robot detected: Collision avoidance activated
                            id=np.where(i_readings-cdd< 0); dread=i_readings[id];dang=i_angles[id]

                            if m==0: #Initialization
                                print('Robot detected at time t=',t)

                                #Estimate robot 2's position in the odom frame                              
                                print(pr2,t)
                                ...
                                #Estimate robot 2's position in the odom frame
                                if (m%(2*MS)) == 0:print(pr2,t)

                                #Estimate robot 2's velocity in the odom frame
