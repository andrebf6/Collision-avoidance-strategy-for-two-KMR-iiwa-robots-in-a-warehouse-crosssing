...
def listener_callback2(odom_data2):
    xr1= odom_data2.pose.pose.position.x
    yr1= odom_data2.pose.pose.position.y
    orienx1= odom_data2.pose.pose.orientation.x
    orieny1= odom_data2.pose.pose.orientation.y
    orienz1= odom_data2.pose.pose.orientation.z
    orienw1= odom_data2.pose.pose.orientation.w

    siny_cosp = 2 * (orienw1 * orienz1 + orienx1 * orieny1)
    cosy_cosp = 1 - 2 * (orieny1 * orieny1 + orienz1 * orienz1)
    yawc1 = np.arctan2(siny_cosp, cosy_cosp)

    vxr1= odom_data2.twist.twist.linear.x
    vyr1= odom_data2.twist.twist.linear.y

    pr1_real[0]=xr1; pr1_real[1]=yr1; yaw1[0]= yawc1
    v1_real[0]=vxr1; v1_real[1]=vyr1
...
if __name__=="__main__":# Proteger parte del código. Assegura que el bloque solo se ejecutará si tu módulo es el programa principal.
   ...
   # The node is subscribed to the topic /odom, and recives messages of the type Pose and Twist, with a "queue size" of 10 queued messages
    sub3 = node.create_subscription(Odometry, '/robot1/odom',listener_callback2, 10)
    ...
    # Real values (Eliminate):
    pr1_real=np.ones(2)*2000                        # Position of robot 1's centre of mass in the odom frame  (m)
    v1_real=np.ones(2)*2000                         # Velocity of robot 1's centre of mass in the odom frame  (m)
    yaw1=np.ones(1)*2000                            # Orientation of the robot 1's in the 2D plane (rad)
    rotz1=np.zeros((2,2))*2000                      # Initialize rotation matrix (change from the odom to the base-footprint frame)
                    ...
                    if pr2[0]!=2000 and pr2[1]!=2000 and scan==True: # Position information has been published at least once and a new sensor reading has entred.
                        ...
                        if np.any(i_readings-cdd < 0)==True:
                            ...
                            if m==0: #Initialization
                                ...
                                #print(pr1,dv2_o[0]*speed2, phi*57.2957795131, oc*57.2957795131, res*57.2957795131)
                                ...
                                print(pr1_real,pr1,t)
                                pr1=pr1_real        # Eliminar
                                ...
                                #Estimate robot 1's velocity---> MOURE DE LLOC!!
                                v1=SF*(pr1-opr1)/(t-t_old)

                                # Eliminar
                                rotz1[0,0]=math.cos(yaw1[0]); rotz1[0,1]=-math.sin(yaw1[0])        # Build rotation matrix (to change from the odom to the base-footprint frame)        
                                rotz1[1,0]=math.sin(yaw1[0]); rotz1[1,1]=math.cos(yaw1[0])
                                v1_real=rotz1.dot(v1_real)
                                ...
                                if pr1[0]<2.5 and pr1[0]>-0.5 and pr1[1]>1.1 and pr1[1]<3.2: # robot 1 is in region A
                                    ...
                                    #Estimate robot 1's velocity --> AFEGIR!!!
